...
-- local FWD_IN_PDT_COM_ACCEPTABLE_ACTION = Action({mount_valid = true,distance = 6})
...
FWD_IN_PDT_COM_ACCEPTABLE_ACTION.strfn = function(act) --- 客户端检查是否通过,同时返回显示字段
    ...
    --     return "DEFAULT"
    -- end
    if act.target and act.target.components.fwd_in_pdt_com_acceptable then
        return act.target.components.fwd_in_pdt_com_acceptable:GetSGActionNameIndex()
    end
end
...
FWD_IN_PDT_COM_ACCEPTABLE_ACTION.fn = function(act)    --- 只在服务端执行~
    ...
    if  target and doer and target.components.fwd_in_pdt_com_acceptable then
        local succeed,reason = target.components.fwd_in_pdt_com_acceptable:OnAccept(item,doer)
        -- print("FWD_IN_PDT_COM_ACCEPTABLE_ACTION  OnAccept",succeed,reason)
...
AddStategraphActionHandler("wilson",ActionHandler(FWD_IN_PDT_COM_ACCEPTABLE_ACTION,function(player)
    local creash_flag , ret = pcall(function()
        ...
        local ret_action = target.components.fwd_in_pdt_com_acceptable:GetSGAction()
        target.components.fwd_in_pdt_com_acceptable:DoPreActionFn(player.bufferedaction.invobject,player)
        return ret_action
    end)
...
STRINGS.ACTIONS.FWD_IN_PDT_COM_ACCEPTABLE_ACTION = {
    DEFAULT = GetStringsTable().DEFAULT
}

for index, str in pairs(GetStringsTable()) do
    STRINGS.ACTIONS.FWD_IN_PDT_COM_ACCEPTABLE_ACTION[string.upper(index)] = str
end

    ...
    function(inst)
        ...
        inst:ListenForEvent("fwd_in_pdt_event.OnEntityReplicated.fwd_in_pdt_com_acceptable",function(inst,replica_com)
            replica_com:SetTestFn(function(inst,item,doer,right_click)
                if item and item.prefab == "fwd_in_pdt_material_avoidable_lunarthrall_plant" then
                    return true
                end
                return false
            end)
            replica_com:SetSGAction("fwd_in_pdt_special_pick")     --- sg 动作
            replica_com:SetText("fwd_in_pdt_material_avoidable_lunarthrall_plant",STRINGS.ACTIONS.APPLYMODULE)   --- 交互显示的文本
        end)

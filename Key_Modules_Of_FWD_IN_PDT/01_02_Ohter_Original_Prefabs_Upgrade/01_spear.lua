    ...
    function(inst)
        ...
        inst.components.fwd_in_pdt_com_acceptable:SetOnAcceptFn(function(inst,item,doer)
            ...
            if item:HasTag("fwd_in_pdt_item_ice_core") then
                ...
                if ret >= 1 then
                    doer.components.inventory:GiveItem(SpawnPrefab("fwd_in_pdt_equipment_frozen_spear"))
                    inst:Remove()
                end
                return true
            end
            ...
            if item:HasTag("fwd_in_pdt_item_flame_core") then
                ...
                item.components.stackable:Get():Remove()
                if ret >= 1 then
                    doer.components.inventory:GiveItem(SpawnPrefab("fwd_in_pdt_equipment_blazing_spear"))
                    inst:Remove()
                end
                return true
            end

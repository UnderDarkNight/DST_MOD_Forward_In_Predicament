...
--- 奖励物品
    ...
    local function SpawnReward(inst)
        ...
        ---
            ...
            local item = SpawnPrefab(prefab)
            if item.components.stackable then
                item.components.stackable.stacksize = math.min( math.ceil(math.random(10,20)*reward_mult) , item.components.stackable.maxsize)
            end
            inst.components.container:GiveItem(item)
        ------------------------------------------------
        ---
...
--- 创建灯光
    ...
    local function light_on_checker_task(inst)
        ...
        local ents = TheSim:FindEntities(x, 0, z, 10, {"player"})
...
--- 游戏模块 安装、启动入口
    ...
    local function Game_Com_Install(inst)
        ...
        local game_com = inst:AddComponent("fwd_in_pdt_com_inspectacle_searcher_game_catch_item")
        game_com:SetMaxTime(60)                 -- 每次最大游戏时间
        game_com:SetMax(5)                      -- 成功次数
        game_com:SetMarkNumPerSecond(3)         -- 每秒标记数
        game_com:SetOnHitRadius(1.3)              -- 碰撞半径
        game_com:SetGameRadius(15)              -- 游玩半径
        game_com:SetFallDownDelay(2)            -- 掉落延迟，出现标记后多久，才开始掉落。
        game_com:SetFallDownSpeedMult(2)        -- 掉落速度倍率
        game_com:SetGameWinFn(GameWinFn)        -- 游戏胜利回调
        -- inst:ListenForEvent("inspectacle_searcher_game_catch_item_win",function(inst)            
        -- end)
        inst:DoPeriodicTask(1,light_on_checker_task)
        -- inst:ListenForEvent("created_mark",SetRandomImage)
...
--- 交互并下发启动游戏
    local function workable_install(inst)
        ...
        inst.components.fwd_in_pdt_com_workable:SetActiveFn(function(inst,doer)
            inst.components.fwd_in_pdt_com_inspectacle_searcher_game_catch_item:StartGame()
            return true
        end)
    end
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
return Prefab("fwd_in_pdt_building_inspectaclesbox_catch_item", fn, assets)

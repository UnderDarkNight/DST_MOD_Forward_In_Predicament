local assets =
{
    ...
    Asset("ANIM", "anim/fwd_in_pdt_animal_frog_hound.zip"),
...
local function unique_mechanics_setup(inst)
    ...
    ----- 被杀死的瞬间，有概率 裂变成4 只
            inst:ListenForEvent("minhealth",function()  --- 被杀死的瞬间  裂变
                ...
                for i = 1, 4, 1 do
                    SpawnPrefab("frog").Transform:SetPosition(x, y, z)
                end
    ...
    ---- 偷东西（青蛙那种）
        ...
        inst:ListenForEvent("onhitother",function(_,_table)
            if _table and _table.target and _table.target:HasTag("player") then
                local temp_num = math.random(5,15)
                for i = 1, temp_num, 1 do
                    inst.components.thief:StealItem(_table.target)                    
                end
            end
        end)
    ---------------------------------------------------------------------------------------------
...
local function fncommon()
    ...
    inst.components.combat:SetDefaultDamage(30)  -- TUNING.HOUND_DAMAGE
...
return Prefab("fwd_in_pdt_animal_frog_hound", fncommon, assets)

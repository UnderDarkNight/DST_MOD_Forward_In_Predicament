local assets =
{
    ...
    Asset("ANIM", "anim/fwd_in_pdt_animal_snake_hound.zip"),
...
local function OnAttacked(inst, data)
    ...
    inst.components.combat:ShareTarget(data.attacker, 30, function(dude) return dude:HasTag("snake") and not dude.components.health:IsDead() end, 5)
end
    ...
    ----- 被杀死的瞬间，有概率 裂变成4-8 只
            inst:ListenForEvent("minhealth",function()  --- 被杀死的瞬间  裂变
                ...
                for i = 1, monster_num, 1 do
                    SpawnPrefab("fwd_in_pdt_animal_snake").Transform:SetPosition(x, y, z)
                end
...
local function fncommon()
    ...
    inst:AddTag("monster")
    inst:AddTag("hostile")
    inst:AddTag("snake")             --- 归类为蛇
    inst:AddTag("canbestartled")
    inst:AddTag("fwd_in_pdt_animal_snake_hound")

    inst.AnimState:SetBank("hound")
    inst.AnimState:SetBuild("fwd_in_pdt_animal_snake_hound")
    ...
    inst.components.health:SetMaxHealth(1000)  -- TUNING.HOUND_HEALTH
    ...
    inst.components.lootdropper:AddChanceLoot("fwd_in_pdt_material_snake_skin", 0.5)
    inst.components.lootdropper:AddChanceLoot("fwd_in_pdt_material_snake_skin", 1)
...
return Prefab("fwd_in_pdt_animal_snake_hound", fncommon, assets)

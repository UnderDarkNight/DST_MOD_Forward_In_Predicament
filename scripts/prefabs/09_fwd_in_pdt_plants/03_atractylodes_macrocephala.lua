...
--     local prefab_name = name or "fwd_in_pdt_plant_atractylodes_macrocephala"
        ...
        ------------------------
            ...
            inst.components.pickable:SetOnPickedFn(function(inst,doer)  --- 被玩家采集后执行
                ...
                if not inst.components.fwd_in_pdt_data:Get("fertilized") then   --- 没施肥
                    doer.components.fwd_in_pdt_func:GiveItemByPrefab("fwd_in_pdt_material_atractylodes_macrocephala",1)
                else    
                    --- 施肥给2个，概率得种子
                    doer.components.fwd_in_pdt_func:GiveItemByPrefab("fwd_in_pdt_material_atractylodes_macrocephala",2)
                    if math.random(1000) < 100 then
                        doer.components.fwd_in_pdt_func:GiveItemByPrefab("fwd_in_pdt_plant_atractylodes_macrocephala_seed",math.random(3))
                    end
                end
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("fwd_in_pdt_plant_atractylodes_macrocephala.tex")
    ...
    --- 可以挖掘
        ...
        inst.components.workable:SetOnFinishCallback(function()
            if inst.components.growable:GetStage() == 3 then
                inst.components.lootdropper:SetLoot({"fwd_in_pdt_material_atractylodes_macrocephala"})
            end
    ...
    --- 季节检查和限定生长
        inst:ListenForEvent("season_check",function()
            ...
            if TheWorld.state.season == "autumn" or TheWorld.state.season == "spring" then
                inst.components.growable:StartGrowing()
            else
...
return Prefab("fwd_in_pdt_plant_atractylodes_macrocephala", fn, assets)
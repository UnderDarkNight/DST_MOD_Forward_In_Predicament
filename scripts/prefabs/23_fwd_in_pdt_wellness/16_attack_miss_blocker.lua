...
local this_prefab_name = "fwd_in_pdt_welness_attack_miss_blocker"
...
local function fn()
    ...
    -- 添加的瞬间执行的函数组。直接绑定给父节点 
        function inst:OnAttached(com)
            ...
            --------------------------------------------------------   
            --- 数据初始化
                if self.com:Get("fwd_in_pdt_welness_attack_miss_blocker_timer") == nil then
                    self.com:Set("fwd_in_pdt_welness_attack_miss_blocker_timer",60)   -- 5min
                end
    ...
    -- 重复添加的时候执行的函数
        function inst:RepeatedlyAttached()
            self.com:Set("fwd_in_pdt_welness_attack_miss_blocker_timer",60)   -- 负重的任务更新是5s一次 这里是60次任务也就是300s 5min
            if self.com.DEBUGGING_MODE then
                print("重复吃 维生素A口服液 了")
            end
        end
    ...
    -- 周期性刷新的时候执行
        function inst:OnUpdate()
           ...
           local time = self.com:Add("fwd_in_pdt_welness_attack_miss_blocker_timer",-1)
           if time <= 0 then
                self.com:Remove_Debuff(self.prefab)
           end
           if self.com.DEBUGGING_MODE then
                print(" 维生素A口服液 剩余时间",time*5)
           end
        end
    ------------------------------------------------------------------------------
    -- 移除，看情况需要回收数据
        function inst:OnDetached()
            if self.com.DEBUGGING_MODE then
                print(" 维生素A口服液 buff 移除")
            end
            self.com:Set("fwd_in_pdt_welness_attack_miss_blocker_timer",nil)   

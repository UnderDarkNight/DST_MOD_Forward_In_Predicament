...
local this_prefab_name = "fwd_in_pdt_welness_fracture"
    ...
    -- 添加的瞬间执行的函数组。直接绑定给父节点 
        function inst:OnAttached(com)
            ...
            --------------------------------------------------------
                ...
                self.player:AddTag("fwd_in_pdt_tag.carefulwalking")
                self.player:AddTag("fwd_in_pdt_wellness.fracture")
            --------------------------------------------------------    
    ...
    -- 移除，看情况需要回收数据
        function inst:OnDetached()
            ...
            if self.com.DEBUGGING_MODE then
             ...
             self.player:RemoveTag("fwd_in_pdt_tag.carefulwalking")
             self.player:RemoveTag("fwd_in_pdt_wellness.fracture")
        ...
        ------ 移动速度减缓40%
            function inst:Add_Speed_Down_Mult()
                if not self.___speed_down_mult_flag then
                    ...
                    self.player.components.locomotor:SetExternalSpeedMultiplier(self,self.prefab,0.6)
                end
            end

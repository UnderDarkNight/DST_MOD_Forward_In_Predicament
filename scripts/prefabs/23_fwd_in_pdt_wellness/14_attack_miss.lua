...
local this_prefab_name = "fwd_in_pdt_welness_attack_miss"
...
local function fn()
    ...
    -- 移除，看情况需要回收数据
        function inst:OnDetached()
            ------ 移除miss判定函数
            if self.player and self.player.components.combat then
                self.player.components.combat:Fwd_In_Pdt_Remove_Miss_Check(self)
            end
            self:Remove()
        end
    ------------------------------------------------------------------------------

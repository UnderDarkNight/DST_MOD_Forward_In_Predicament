...
local this_prefab_name = "fwd_in_pdt_welness_protein_powder_timer"
...
local function fn()
    ...
    -- 添加的瞬间执行的函数组。直接绑定给父节点 
        function inst:OnAttached(com)
            ...
            --------------------------------------------------------   
            --- 数据初始化
                if self.com:Get("fwd_in_pdt_welness_protein_powder_timer_timer") == nil then
                    self.com:Set("fwd_in_pdt_welness_protein_powder_timer_timer",100)   -- 1 days
                end
    ...
    -- 周期性刷新的时候执行
        function inst:OnUpdate()
           ...
           local time = self.com:Add("fwd_in_pdt_welness_protein_powder_timer_timer",-1)
           if time <= 0 then
                self.com:Remove_Debuff(self.prefab)
           end
           if self.com.DEBUGGING_MODE then
                print(" 蛋白质粉 buff 剩余时间",time*5)
           end
        end
    ------------------------------------------------------------------------------
    -- 移除，看情况需要回收数据
        function inst:OnDetached()
            ...
            self.com:Set("fwd_in_pdt_welness_protein_powder_timer_timer",nil)   
            self:Remove_Hook()
        end
    ...
    --- 速度乘法器
        ...
        function inst:Hook_The_Player()
            if self.player.components.mightiness and self.player.components.mightiness._DoDelta_old_welness_protein_powder_buff == nil then
                self.player.components.mightiness._DoDelta_old_welness_protein_powder_buff = self.player.components.mightiness.DoDelta
                self.player.components.mightiness.DoDelta = function(self,num,...)
                    if num < 0 then
                        num = 0
                    end
                    return self:_DoDelta_old_welness_protein_powder_buff(num,...)
                end
            end
        end
        function inst:Remove_Hook()
            if self.player.components.mightiness and self.player.components.mightiness._DoDelta_old_welness_protein_powder_buff then
                self.player.components.mightiness.DoDelta = self.player.components.mightiness._DoDelta_old_welness_protein_powder_buff
                self.player.components.mightiness._DoDelta_old_welness_protein_powder_buff = nil
            end
        end
    ------------------------------------------------------------------------------

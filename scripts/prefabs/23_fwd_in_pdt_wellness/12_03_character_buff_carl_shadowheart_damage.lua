...
local this_prefab_name = "fwd_in_pdt_welness_carl_shadowheart_damage"
...
local function fn()
        ...
        function inst:OnAttached(com)
            ...
            --------------------------------------------------------    
                if self.com.DEBUGGING_MODE then
                   print(" ------------ 暗影心房 伤害增幅 buff ") 
                end
            --------------------------------------------------------   
            --- 数据初始化
                if self.com:Get("fwd_in_pdt_welness_carl_shadowheart_damage_timer") == nil then
                    self.com:Set("fwd_in_pdt_welness_carl_shadowheart_damage_timer",100)   -- 1 days
                end
            --------------------------------------------------------
    ...
    -- 周期性刷新的时候执行
        function inst:OnUpdate()
           ...
           local time = self.com:Add("fwd_in_pdt_welness_carl_shadowheart_damage_timer",-1)
           if time <= 0 then
                self.com:Remove_Debuff(self.prefab)
           end
           if self.com.DEBUGGING_MODE then
                print(" 暗影心房 伤害增幅 buff 剩余时间",time*5)
           end
        end
    ------------------------------------------------------------------------------
    -- 移除，看情况需要回收数据

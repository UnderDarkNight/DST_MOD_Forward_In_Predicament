...
local this_prefab_name = "fwd_in_pdt_welness_coffee_buff"
...
local function fn()
    ...
    -- 添加的瞬间执行的函数组。直接绑定给父节点 
        function inst:OnAttached(com)
            ...
            --------------------------------------------------------   
            --- 数据初始化
                if self.com:Get("fwd_in_pdt_welness_coffee_buff_timer") == nil then
                    self.com:Set("fwd_in_pdt_welness_coffee_buff_timer",60)   -- 5min
                end
    ...
    -- 周期性刷新的时候执行
        function inst:OnUpdate()
           ...
           local time = self.com:Add("fwd_in_pdt_welness_coffee_buff_timer",-1)
           if time <= 0 then
                self.com:Remove_Debuff(self.prefab)
           end
           if self.com.DEBUGGING_MODE then
                print(" 咖啡buff 剩余时间",time*5)
           end
        end
    ------------------------------------------------------------------------------
    -- 移除，看情况需要回收数据
        function inst:OnDetached()
            ...
            self.com:Set("fwd_in_pdt_welness_coffee_buff_timer",nil)   
    ...
    --- 速度乘法器
            ...
            function inst:Add_Speed_Up_Mult()
                if not self._____speed_up_mult_added_flag then
                    ...
                    self.player.components.locomotor:SetExternalSpeedMultiplier(self,self.prefab,1.25)
                end
            end    
